...
void TextureData::setupTexturePNG(unsigned int unit, std::string texfilepath)
{
	...
	GLCALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, imgWidth, imgHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, imgData));
	GLCALL(glGenerateMipmap(GL_TEXTURE_2D)); //#FIX_URGENT Throws Exceptions randomly.
	stbi_image_free(imgData);
}
