...
public:
	...
	void Render(std::shared_ptr<I_Renderer> renderer, std::shared_ptr<I_API_Manager> apiManager, std::shared_ptr<ECSCoordinator> ECScoord, float deltaTime, float time, int clippingPlane, float& offsetVal)
	{	
		...
		for (auto const& EID : m_EntitySet)
		{
			renderer->Render(apiManager->GetEntityDataHandle(EID), *ECScoord, EID, deltaTime, time, clippingPlane, offsetVal);
		}
	}
