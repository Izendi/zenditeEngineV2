...
public:
	...
	void Render(std::shared_ptr<I_Renderer> renderer, std::shared_ptr<I_API_Manager> apiManager, std::shared_ptr<ECSCoordinator> ECScoord, float deltaTime, float time, int clippingPlane, float& offsetVal, float r, float g, float b, int renderPass, bool& castShadows, float& shadowIntensity)
	{	
		...
		for (auto const& EID : m_EntitySet)
		{
			renderer->Render(apiManager->GetEntityDataHandle(EID), *ECScoord, EID, deltaTime, time, clippingPlane, offsetVal, r, g, b, renderPass, castShadows, shadowIntensity);
		}
	}
